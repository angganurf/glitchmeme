...
function EmptyFolder({
	...
	const recycleBin = desktopIcon.filter(
		(icon) => icon.folderId === "RecycleBin"
	);
	...
	return (
		<Draggable
			...
			defaultPosition={{
				...
				y: window.innerWidth <= 500 ? 300 : 360,
			}}
			onStop={(event, data) => {
		...
		>
			<motion.div
				...
				className={`folder_folder ${
					photoMode ? "photo_mode" : paintMode ? "paint_mode" : ""
				}`}
				onClick={(e) => {
					e.stopPropagation();
					handleSetFocusItemTrue(folderName);
					...
					<div className="folder_barname">
						<img
							src={
								photoMode
									? photoicon
									: folderName === "RecycleBin" && recycleBinLength === 0
									? binEmp
									: folderName === "RecycleBin" && recycleBinLength > 0
									? bin
					...
					<div className="folder_barbtn">
						...
						<div>
							<p
								className="x"
								onClick={
									!isTouchDevice ? () => deleteTap(folderName) : undefined
								}
						...
						>
							...
							>
								...
								>
									...
									{desktopIcon
										.filter((icon) => icon.folderId === folderName)
										.map((icon) => (
