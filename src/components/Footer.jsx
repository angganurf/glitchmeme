...
export default function Footer() {
	...
	const footerItems = [
		...
		},
		{
			...
			spanText: "X",
			onClick: () => {
				handleDoubleClickEnterLink("X", handleShow);
	...
	useEffect(() => {
		const handleMouseMove = (event) => {
			...
			if (startPopupContainer) {
				...
				if (
					isMouseOutsideStart &&
					isMouseOutsideProject &&
					isMouseOutsideResume
				) {
					setProjectStartBar(false);
					setResumejectStartBar(false);
				}
	...
	const projectFolderItem = desktopIcon.filter(
		(icon) => icon.folderId === "Project"
	).length;
	const resumeFolderItem = desktopIcon.filter(
		(icon) => icon.folderId === "Resume"
	).length;
