...
function MyComputer() {
	...
	// popup select folder
	const popUpiconList = [
		// important default folderand disks
		{ name: "MyComputer", pic: imageMapping("MyComputer"), at: "MyComputer" },
		{
			name: "Hard Disk (C:)",
			pic: imageMapping("Hard Disk (C:)"),
			at: "DiskC",
		},
		{
			name: "Hard Disk (D:)",
			pic: imageMapping("Hard Disk (D:)"),
			at: "DiskD",
		},
	...
	const folderMap = [
		...
		},
		{
			folder: "DiskC",
			label: "Hard Disk (C:)",
			img: imageMapping("Hard Disk (C:)"),
		},
		{
			folder: "DiskD",
			label: "Hard Disk (D:)",
			img: imageMapping("Hard Disk (D:)"),
		},
	...
	function UndoFunction() {
		...
		const selectedFolder = folderMap.find(
			(item) => item.folder === updatedUndo[updatedUndo.length - 1]
		);
		...
		>
			...
			>
				<div
					...
					style={{
						background: MyComputerExpand.focusItem ? themeDragBar : "#757579",
					}}
				>
					<div className="folder_barname">
						<img
							src={imageMapping(selectedFolder.label)}
					...
					<div className="folder_barbtn">
						...
						>
							<motion.div
								className={`expand ${MyComputerExpand.expand ? "full" : ""}`}
							/>
							{MyComputerExpand.expand ? (
						...
						<div>
							<p
								className="x"
								onClick={
									!isTouchDevice ? () => deleteTap("MyComputer") : undefined
								}
				...
				<div className="drive_control">
					<div className="drive_link">
									...
									<ul>
										{popUpiconList.map((icon, index) => (
											<>
												...
												>
													<img
														src={imageMapping(icon.name)}
														alt={icon.name}
														style={{
															marginLeft: MarginOnSelectedIcon(icon.name),
														}}
													/>
													<span>{icon.name}</span>
												</li>
													...
													(subFolder, index) =>
														subFolder.folderId === icon.at && (
															<>
																<li
																	...
																	onClick={() => {
																		setSelectedFolder({
																			...
																			img: imageMapping(subFolder.name),
																		});
																		setPopUpFolder(false);
																...
																>
																	<img
																		src={imageMapping(subFolder.name)}
																		alt={subFolder.name}
																		style={{
																			marginLeft: MarginOnSelectedIcon(
																				subFolder.name,
																				"sub1"
																			),
																		}}
																...
																{subFolders.map((subSubFolder, index) =>
																	subSubFolder.folderId === subFolder.name ? (
																		<li
																			...
																			onClick={() => {
																				setSelectedFolder({
																					...
																					img: imageMapping(subSubFolder.name),
																				});
																				setPopUpFolder(false);
																		...
																		>
																			<img
																				src={imageMapping(subSubFolder.name)}
																				alt={subSubFolder.name}
																				style={{
																					marginLeft: MarginOnSelectedIcon(
																						subSubFolder.name,
																						"sub2"
						...
						<div className="folder_select_left_container">
							...
							<p>{selectedFolder.label}</p>
						</div>
						<div
							className="folder_select_btn"
							onClick={() => {
				...
				>
					<div
						className="parent_item_container"
						key={key}
					...
					>
						...
						>
							{desktopIcon
								.filter((icon) => icon.folderId === currentFolder)
								.map((icon) => (
				...
				<div className="btm_bar_container">
					<div className="object_bar">
						<p>
							{desktopIcon
								.filter((icon) => icon.folderId === currentFolder)
								.some((icon) => icon.focus)
								? "1 object(s) selected"
								: desktopIcon.filter((icon) => icon.folderId === currentFolder)
										.length +
								  " " +
								  "object(s)"}
						</p>
					...
					<div className="size_bar">
						<p>
							{(() => {
								const filteredIcons = desktopIcon.filter(
									(icon) => icon.folderId === currentFolder
								);
								const totalSize = filteredIcons.reduce(
									(total, icon) => total + icon.size,
									0
								);
								const allNotFocused = filteredIcons.every(
									(icon) => !icon.focus
								);
