...
function BgSetting() {
	...
	const [localEffect, setLocalEffect] = useState(() => {
		...
		return prevEffect ? prevEffect : eff6;
	});
	...
	useEffect(() => {
		// force set background and effect when app opened for color picker
		if (userPickedColor) {
	...
	function setbgColorFunction2Effect(index) {
		const selectedOption = effectOptions.find(
			(option) => option.value === index
		);
	...
	useEffect(() => {
		// when exited app, make set everything to null to prevent bug when reopen
	...
	function appleBG() {
		...
		if (ImgBgPreviewEffect) {
			// for Effect
			rootEffect.style.setProperty(
				"--before-bg-image",
				`url(${ImgBgPreviewEffect})`
			);
		}
		...
		<>
			...
			>
				<motion.div
					...
					style={
						BgSettingExpand.expand
							? inlineStyleExpand("Settings")
							: inlineStyle("Settings")
					}
				>
					<div
						className="folder_dragbar_bgsetting"
						style={{
							background: BgSettingExpand.focusItem ? themeDragBar : "#757579",
						}}
					>
					...
					<div className="folder_content">
						...
						{bgTap && (
							<div className="folder_content-bgsetting">
								...
								<div
									...
									style={{
										backgroundColor: userPickedColor ? userPickedColor : "",
									}}
								>
									{ImgBgPreview && <img src={ImgBgPreview} alt="" />}
								</div>
								{pickerPanel && (
									<div
										...
										<div className="wallpaper_container">
											{effectOptions.map((option) => (
												<ul
													key={option.value}
													onClick={() =>
														setbgColorFunction2Effect(option.value)
													}
													style={
						...
						<div className="bgsetting_btn_container">
							...
							<div
								className="bgsetting_btn_cancel"
								onClick={
									!isTouchDevice
										? () => {
												deleteTap("Settings");
							...
							<div className="bgsetting_btn_cancel" onClick={appleBG}>
								<span>Apply</span>
							</div>
						</div>
					</div>
