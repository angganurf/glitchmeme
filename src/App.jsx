...
function App() {
	...
	const [themeDragBar, setThemeDragBar] = useState(
		() => localStorage.getItem("barcolor") || "#0100c0"
	);
	...
	const [DexscreenerExpand, setDexscreenerExpand] = useState({
		expand: false,
		show: false,
		hide: false,
		focusItem: true,
	...
	const textError = // error message
		(
			<>
				Cannot find the file '{RunInputVal || regErrorPopUpVal}' (or one of its
				component). Make sure the path and filename are correct and that all
	...
	function ObjectState() {
		...
		return [
			...
			{
				name: "ResumeFile",
				setter: setResumeFileExpand,
				usestate: ResumeFileExpand,
			},
			{
				name: "MineSweeper",
	...
	function handleShow(name) {
		...
		const itemExists = allSetItems.some(
			(item) => item.name.toLowerCase().split(" ").join("") === lowerCaseName
		);
	...
	function inlineStyleExpand(name) {
		...
		if (item) {
			return {
				...
				left: `${
					item.usestate.x <= 0
						? Math.abs(item.usestate.x) * 2 + item.usestate.x
						: -item.usestate.x
				}px`,
				top: `${
					item.usestate.y <= 0
						? Math.abs(item.usestate.y) * 2 + item.usestate.y
						: -item.usestate.y
				}px`,
